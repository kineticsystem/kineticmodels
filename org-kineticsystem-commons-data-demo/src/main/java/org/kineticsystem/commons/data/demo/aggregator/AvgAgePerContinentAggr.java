 ...
 * @version $Revision: 26 $
 */
public class AvgAgePerContinentAggr implements GroupAggregator<RandomContact> {

    public Object getValue(java.util.List<RandomContact> contacts) {
        int val = 0;
        Iterator iter = contacts.iterator();
        while (iter.hasNext()) {
            val += ((RandomContact) iter.next()).getAge();
        }
        int result = (int) Math.round(val / contacts.size());
    ...
    public String toString() {
        return "Average age per continent";
    }
}
