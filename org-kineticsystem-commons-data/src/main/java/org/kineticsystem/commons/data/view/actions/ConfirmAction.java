...
/**
 * Action connected to an editor controller and used to confirm an executeChange, an
 * insertion or a removal of an object.
 * @author Giovanni Remigi
 * @version $Revision: 145 $
 * @see org.kineticsystem.commons.data.controller.DataNavigator
 */
public class ConfirmAction extends AdvancedAction {
    
    /**
     * Name of the action used to confirm an executeChange, an insertion or a
     * removal of an object.
     */
    public static final String ACTION_NAME = "ConfirmAction";
     ...
     */
    public ConfirmAction(DataNavigator controller) {
        this.controller = controller;
        putValue(Action.SMALL_ICON,
            ResourceLoader.getIcon(NAVIGATOR_RESOURCE + "Save16.png"));
        putValue(Action.NAME, Localizer.localizeString(NAVIGATOR_BUNDLE,
            "ConfirmAction"));
        putValue(Action.SHORT_DESCRIPTION,               
            Localizer.localizeString(NAVIGATOR_BUNDLE,
                "ConfirmAction_Description"));
        setEnabled(false);
    }
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        if (controller.getState() == DataNavigator.CREATION_STATE) {
            controller.executeCreation();
        } else if (controller.getState() == DataNavigator.EDITING_STATE){
