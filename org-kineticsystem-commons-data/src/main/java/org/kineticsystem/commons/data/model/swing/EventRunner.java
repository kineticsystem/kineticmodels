 ...
 * @version $Revision: 43 $
 */
class EventRunner<E> implements Runnable, ActiveListListener {
    ...
    /**
     * Constructor.
     * @param list The virtual list, composed by two embedded list, that's seen
     *     by the user.
     * @param srcList The embedded <tt>ActiveList</tt> list sending events whose
     *     effetcs must be under the control of the AWT event-dispatching
     *     thread.
     * @param dstList The embedded list that is seen by the Swing componens
     *     throw the virtual list.
     */
    public EventRunner(DataList<E> list, ActiveList<E> srcList,
            ActiveList<E> dtsList) {
    ...
    public void run() {
        ...
        try {
            ...
            boolean activated = list.isActive();
            list.setActive(false);
            
            /*
             * IMPORTANT
             * With JRE-5 the unlock was here. JRE-6 introduced new features for
             * ordering tables that force the unlock to be moved after the Swing
             * event notification.
             */
            ...
            while (iter.hasNext()) {
                ...
                ActiveListUtility.synchronize(srcList, dstList, event);
                
                // Change event source list and fire the new event.
    
                ActiveListEvent bridgeEvent = new ActiveListEvent(list);
                ...
                list.fireContentsChanged(bridgeEvent);
            }
